...
import { HiHome, HiOfficeBuilding, HiClock, HiCreditCard, HiCash, HiChartBar, HiSpeakerphone, HiCog, HiChevronLeft, HiChevronRight, HiX } from 'react-icons/hi';

const navItems = [
  { to: '/bizhub', icon: HiHome, label: 'Dashboard', end: true },
  { to: '/bizhub/tenants', icon: HiOfficeBuilding, label: 'Tenants' },
  { to: '/bizhub/approvals', icon: HiClock, label: 'Approvals' },
  { to: '/bizhub/plans', icon: HiCreditCard, label: 'Plans' },
  { to: '/bizhub/payments', icon: HiCash, label: 'Payments' },
  { to: '/bizhub/analytics', icon: HiChartBar, label: 'Analytics' },
  { to: '/bizhub/communication', icon: HiSpeakerphone, label: 'Communication' },
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 z-50 h-full bg-[var(--sidebar-bg)] border-r border-[var(--border-color)] transition-all duration-300 flex flex-col ${sidebarOpen ? 'w-64' : 'w-20'} ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="h-16 flex items-center justify-between px-4 border-b border-[var(--border-color)] bg-teal-600">
          {sidebarOpen && <span className="text-lg font-bold text-white">BizHub</span>}
            ...
            {navItems.map(item => (
              <li key={item.to}>
                <NavLink to={item.to} end={item.end} onClick={() => setMobileSidebarOpen(false)}
                  className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-teal-50 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400' : 'text-[var(--text-secondary)] hover:bg-[var(--sidebar-hover)]'}`}>
        ...
        {sidebarOpen && <div className="p-4 border-t border-[var(--border-color)] text-xs text-[var(--text-muted)] text-center">BizHub v3.0</div>}
      </aside>
    </>
  );
}