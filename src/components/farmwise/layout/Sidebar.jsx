...
import { useThemeSidebar } from '../../../context/farmwise/SidebarContext';
import { HiHome, HiUsers, HiCog, HiHeart, HiChevronLeft, HiChevronRight, HiX } from 'react-icons/hi';

const navItems = [
  { to: '/farmwise', icon: HiHome, label: 'Dashboard', end: true },
  { to: '/farmwise/users', icon: HiUsers, label: 'Farmers' },
  { to: '/farmwise/settings', icon: HiCog, label: 'Settings' },
  { to: '/farmwise/health', icon: HiHeart, label: 'Health' },
];
    ...
    <>
      ...
      <aside className={`fixed top-0 left-0 z-50 h-full bg-[var(--sidebar-bg)] border-r border-[var(--border-color)] transition-all duration-300 flex flex-col ${sidebarOpen ? 'w-64' : 'w-20'} ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="h-16 flex items-center justify-between px-4 border-b border-[var(--border-color)] bg-emerald-600">
          {sidebarOpen && <span className="text-lg font-bold text-white">FarmWise</span>}
            ...
            {navItems.map(item => (
              <li key={item.to}>
                <NavLink to={item.to} end={item.end} onClick={() => setMobileSidebarOpen(false)}
                  className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'text-[var(--text-secondary)] hover:bg-[var(--sidebar-hover)]'}`}>
        ...
        {sidebarOpen && <div className="p-4 border-t border-[var(--border-color)] text-xs text-[var(--text-muted)] text-center">FarmWise v1.0</div>}
      </aside>
    </>
  );
}