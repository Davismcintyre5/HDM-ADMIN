...
import { useThemeSidebar } from '../../../context/eduprime/SidebarContext';
import {
  HiHome, HiAcademicCap, HiClock, HiSupport, HiScale,
  HiDatabase, HiClipboardList, HiHeart, HiCog,
  HiChevronLeft, HiChevronRight, HiX
} from 'react-icons/hi';

const navItems = [
  { to: '/eduprime', icon: HiHome, label: 'Dashboard', end: true },
  { to: '/eduprime/schools', icon: HiAcademicCap, label: 'Schools' },
  { to: '/eduprime/pending', icon: HiClock, label: 'Pending' },
  { to: '/eduprime/support', icon: HiSupport, label: 'Support' },
  { to: '/eduprime/legals', icon: HiScale, label: 'Legals' },
  { to: '/eduprime/backups', icon: HiDatabase, label: 'Backups' },
  { to: '/eduprime/logs', icon: HiClipboardList, label: 'Logs' },
  { to: '/eduprime/health', icon: HiHeart, label: 'Health' },
  { to: '/eduprime/settings', icon: HiCog, label: 'Settings' },
];
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 z-50 h-full bg-[var(--sidebar-bg)] border-r border-[var(--border-color)] transition-all duration-300 flex flex-col ${sidebarOpen ? 'w-64' : 'w-20'} ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="h-16 flex items-center justify-between px-4 border-b border-[var(--border-color)] bg-amber-600">
          {sidebarOpen && <span className="text-lg font-bold text-white">🎓 EduPrime</span>}
        ...
        {sidebarOpen && <div className="p-4 border-t border-[var(--border-color)] text-xs text-[var(--text-muted)] text-center">EduPrime v1.0</div>}
      </aside>
    </>
  );
}