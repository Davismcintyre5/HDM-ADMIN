...
import { useThemeSidebar } from '../../../context/hdmnet/SidebarContext';
import {
  HiHome, HiUsers, HiCheckCircle, HiCreditCard,
  HiDatabase, HiCog, HiScale, HiChevronLeft, HiChevronRight, HiX, HiWifi
} from 'react-icons/hi';

const navItems = [
  { to: '/hdmnet', icon: HiHome, label: 'Dashboard', end: true },
  { to: '/hdmnet/providers', icon: HiUsers, label: 'Providers' },
  { to: '/hdmnet/pending', icon: HiCheckCircle, label: 'Pending Activations' },
  { to: '/hdmnet/transactions', icon: HiCreditCard, label: 'Transactions' },
  { to: '/hdmnet/backups', icon: HiDatabase, label: 'Backups' },
  { to: '/hdmnet/legal', icon: HiScale, label: 'Legal' },
  { to: '/hdmnet/settings', icon: HiCog, label: 'Settings' },
];
    ...
    <>
      ...
      <aside className={`fixed top-0 left-0 z-50 h-full bg-[var(--sidebar-bg)] border-r border-[var(--border-color)] transition-all duration-300 flex flex-col ${sidebarOpen ? 'w-64' : 'w-20'} ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="h-16 flex items-center justify-between px-4 border-b border-[var(--border-color)] bg-blue-600">
          {sidebarOpen && <span className="text-lg font-bold text-white">📡 HDM NET</span>}
        ...
        <Link to="/" className="mx-3 mb-2 flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-[var(--text-secondary)] hover:bg-[var(--sidebar-hover)] transition-colors">
          <HiWifi className="w-4 h-4 flex-shrink-0" /> {sidebarOpen && 'Return Home'}
        </Link>
        {sidebarOpen && <div className="p-4 border-t border-[var(--border-color)] text-xs text-[var(--text-muted)] text-center">HDM NET v1.0</div>}
      </aside>
    </>
  );
}