...
import { useAuth } from '../../../context/portfolio/AuthContext';
  ...
  return (
    <header className="h-16 bg-[var(--header-bg)] border-b border-[var(--border-color)] flex items-center justify-between px-4 lg:px-6 sticky top-0 z-30 transition-colors duration-200">
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm font-medium text-[var(--text-secondary)] hidden sm:block">Welcome back{user?.name ? `, ${user.name}` : ''}</span>
      </div>
      <div className="flex items-center gap-2">
        ...
        <div className="hidden sm:flex items-center gap-2 pl-2 border-l border-[var(--border-color)]">
          <div className="w-8 h-8 rounded-full bg-green-600 flex items-center justify-center text-white text-sm font-medium">{user?.name?.charAt(0)?.toUpperCase() || 'A'}</div>
          <span className="text-sm text-[var(--text-secondary)] hidden md:block">{user?.name || user?.email || 'Admin'}</span>
        </div>
