    ...
    <div className="flex items-center justify-between gap-2 mt-4">
      ...
      <div className="flex gap-1">
        {pages.map((p, i) => p === '...' ? <span key={`dots-${i}`} className="px-2 py-1 text-[var(--text-muted)] text-sm">...</span> :
          <button key={p} onClick={() => onPageChange(p)} className={`w-8 h-8 rounded-lg text-sm font-medium transition-colors ${p === page ? 'bg-indigo-600 text-white' : 'text-[var(--text-secondary)] hover:bg-[var(--sidebar-hover)]'}`}>{p}</button>
        )}
      </div>
