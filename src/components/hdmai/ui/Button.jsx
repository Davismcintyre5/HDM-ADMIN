export default function Button({ children, variant = 'primary', size = 'md', className = '', loading, disabled, ...props }) {
  ...
  const variants = {
    ...
    outline: 'border border-gray-300 text-gray-700 hover:bg-gray-50 focus:ring-fuchsia-500 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-800',
  ...
  return (
    <button className={`${base} ${variants[variant]} ${sizes[size]} ${className}`} disabled={disabled || loading} {...props}>
      {loading && <Spinner small className="mr-2" />}
      {children}
    </button>
  );
}

function Spinner({ small, className }) {
  return (
    <svg className={`animate-spin ${small ? 'h-4 w-4' : 'h-5 w-5'} ${className}`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
