...
export function useForm(initialValues = {}) {
  ...
  const setFieldError = useCallback((name, error) => setErrors(prev => ({ ...prev, [name]: error })), []);
  const resetForm = useCallback((newValues = initialValues) => { setValues(newValues); setErrors({}); setTouched({}); }, [initialValues]);

  return { values, errors, touched, handleChange, handleBlur, setFieldValue, setFieldError, resetForm, setValues };
}