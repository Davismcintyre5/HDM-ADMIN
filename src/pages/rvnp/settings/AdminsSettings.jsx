...
import { getAdmins, createAdmin, updateAdmin, deleteAdmin } from '../../../services/rvnp/admins';
...
export default function AdminsSettings() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      else await updateAdmin(modal.data._id, data);
  ...
  const columns = [
    ...
    { key: 'actions', label: '', render: row => (
      <div className="flex gap-1">
        ...
        <Button size="sm" variant="danger" onClick={() => setConfirm({ open: true, id: row._id, name: row.name })}><HiTrash className="w-4 h-4" /></Button>
      </div>
    )},
  ];
  ...
  return (
    <div className="max-w-4xl">
      ...
      <ConfirmDialog open={confirm.open} onClose={() => setConfirm({ open: false, id: null, name: '' })} onConfirm={handleDelete}
        title="Delete Admin" message={`Delete ${confirm.name}?`} confirmLabel="Delete" variant="danger" loading={actionLoading} />
    </div>
  );
}