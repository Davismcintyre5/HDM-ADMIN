    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          ...
          <div className="space-y-1">
            {userGrowth.slice(-12).map((d) => (
              <div key={d._id?.month || d._id} className="flex items-center gap-2 text-xs">
                ...
                <div className="flex-1 bg-[var(--bg-tertiary)] rounded-full h-4">
                  <div className="bg-indigo-500 h-4 rounded-full flex items-center justify-end pr-1" style={{ width: `${(d.count / maxGrowth) * 100}%` }}>
        ...
        <Card>
          ...
          <div className="space-y-1">
            {emailVolume.slice(-12).map((d) => (
              <div key={d._id} className="flex items-center gap-2 text-xs">
                <span className="w-20 text-[var(--text-muted)]">{d._id}</span>
                <div className="flex-1 bg-[var(--bg-tertiary)] rounded-full h-4">
                  <div className="bg-indigo-500 h-4 rounded-full flex items-center justify-end pr-1" style={{ width: `${(d.sent / maxVolume) * 100}%` }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <div className="space-y-2">
            {planDist.map((p) => (
              <div key={p._id?.plan} className="flex justify-between text-sm">
