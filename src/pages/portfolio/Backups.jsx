...
import { getBackups, getBackupSettings, updateBackupSettings, createBackup, deleteBackup, restoreBackup } from '../../services/portfolio/backups';
...
export default function Backups() {
  ...
  const handleDownload = async (backup) => {
    try {
      const baseUrl = import.meta.env.VITE_PORTFOLIO_API || 'http://localhost:5000/hdm/api/admin';
      const token = localStorage.getItem('portfolio_token');
      const res = await fetch(`${baseUrl}/backups/${backup._id}/download`, {
  ...
  const columns = [
    { key: 'type', label: 'Type', render: (row) => <Badge variant="green">{row.type || row.backupType || 'manual'}</Badge> },
              ...
              <>
                <div>
                  ...
                  <select value={autoSettings.frequency} onChange={(e) => setAutoSettings(p => ({ ...p, frequency: e.target.value }))}
                    className="w-full px-3 py-2 rounded-lg border border-[var(--border-color)] bg-[var(--input-bg)] text-[var(--text-primary)] text-sm focus:ring-2 focus:ring-green-500">
        ...
        <div className="space-y-4">
          ...
          <label className="flex items-center gap-2 text-sm text-[var(--text-secondary)] cursor-pointer">
            <input type="checkbox" checked={backupForm.includesMedia} onChange={(e) => setBackupForm(p => ({ ...p, includesMedia: e.target.checked }))}
              className="rounded border-[var(--border-color)] text-green-600 focus:ring-green-500" />
            Include Media Files
          </label>
