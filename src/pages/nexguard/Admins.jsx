...
export default function Admins() {
  ...
  const fetchAdmins = () => {
    setLoading(true);
    getAdmins().then(res => setAdmins(res?.data || res || [])).catch(console.error).finally(() => setLoading(false));
  };

  useEffect(() => { fetchAdmins(); }, []);

  const openCreate = () => { setForm({ name: '', email: '', password: '', role: 'moderator' }); setModal({ open: true, mode: 'create', data: null }); };
  const openEdit = (admin) => { setForm({ name: admin.name || '', email: admin.email || '', password: '', role: admin.role || 'moderator' }); setModal({ open: true, mode: 'edit', data: admin }); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setModal({ open: false, mode: 'create', data: null }); fetchAdmins();
  ...
  const roleVariant = { super_admin: 'danger', moderator: 'info', billing_admin: 'warning' };
  ...
  const columns = [
    ...
    { key: 'role', label: 'Role', render: row => <Badge variant={roleVariant[row.role] || 'default'}>{row.role}</Badge> },
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6"><h1 className="text-2xl font-bold text-[var(--text-primary)]">Admins</h1><Button onClick={openCreate}><HiPlus className="w-4 h-4 mr-1" /> Add Admin</Button></div>
      ...
      <Modal open={modal.open} onClose={() => setModal({ open: false, mode: 'create', data: null })} title={modal.mode === 'create' ? 'Create Admin' : 'Edit Admin'} size="lg">
        <div className="space-y-4">
          ...
          <Input label={modal.mode === 'create' ? 'Password' : 'Password (leave empty)'} type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} required={modal.mode === 'create'} />
          <div><label className="block text-sm font-medium text-[var(--text-secondary)] mb-1">Role</label>
            <select value={form.role} onChange={e => setForm({ ...form, role: e.target.value })} className="w-full px-3 py-2 rounded-lg border border-[var(--border-color)] bg-[var(--input-bg)] text-sm">
              {ROLES.map(r => <option key={r} value={r}>{r.replace('_', ' ')}</option>)}</select></div>
      ...
      <ConfirmDialog open={confirm.open} onClose={() => setConfirm({ open: false, id: null, name: '' })} onConfirm={handleDelete} title="Delete Admin" message={`Delete ${confirm.name}?`} confirmLabel="Delete" variant="danger" loading={actionLoading} />
    </div>
  );
}