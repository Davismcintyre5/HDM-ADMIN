...
export default function Payments() {
  ...
  const handleApprove = async () => {
    ...
    try { await approvePayment(confirmApprove.id); setConfirmApprove({ open: false, id: null }); fetchPayments(); } catch (err) { alert(err.message); }
    setActionLoading(false);
  };

  const handleReject = async () => {
    setActionLoading(true);
    try { await rejectPayment(rejectModal.id, reason); setRejectModal({ open: false, id: null }); setReason(''); fetchPayments(); } catch (err) { alert(err.message); }
    setActionLoading(false);
  };
  ...
  return (
    <div>
      ...
      <Card>
        <Table columns={columns} data={data.payments} loading={loading} emptyMessage="No payments found." />
        <Pagination page={page} totalPages={Math.ceil(data.count / 10)} onPageChange={setPage} />
      </Card>
