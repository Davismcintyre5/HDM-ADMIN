...
import { useAuth } from '../../context/hdmerp/AuthContext';
...
export default function Login() {
  ...
  const from = location.state?.from?.pathname || '/hdmerp';
    ...
    <div className="min-h-screen bg-[var(--bg-secondary)] flex items-center justify-center p-4 relative">
      ...
      <div className="w-full max-w-md bg-[var(--card-bg)] rounded-2xl shadow-xl border border-[var(--border-color)] p-6 sm:p-8">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-green-600 dark:text-green-400 mb-1">HDM ERP</h1>
          <p className="text-[var(--text-secondary)] text-sm">Super Admin Panel</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input label="Email" name="email" type="email" value={form.email} onChange={handleChange} placeholder="admin@example.com" />
