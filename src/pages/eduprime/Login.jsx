...
import { useAuth } from '../../context/eduprime/AuthContext';
...
export default function Login() {
  ...
  if (isAuthenticated) return <Navigate to="/eduprime" replace />;

  const handleSubmit = async (e) => {
    e.preventDefault();
    setError(''); setSubmitting(true);
    try { await login(email, password); navigate('/eduprime'); }
      ...
      <Card className="w-full max-w-md">
        <div className="text-center mb-6">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-amber-100 dark:bg-amber-900/30 mb-4">
            <HiAcademicCap className="w-8 h-8 text-amber-600 dark:text-amber-400" />
          </div>
          <h1 className="text-2xl font-bold text-[var(--text-primary)]">EduPrime Admin</h1>
          <p className="text-sm text-[var(--text-secondary)] mt-1">School management & administration</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input label="Email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@eduprime.com" required />
