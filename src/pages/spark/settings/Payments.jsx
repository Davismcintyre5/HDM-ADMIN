...
export default function PaymentsSettings() {
  ...
  const fetchData = () => {
    ...
    Promise.all([getSettings(), getActivations()])
      .then(([s, a]) => {
        setSettings(s);
        setActivations(a.activations || a.data || []);
      })
      .catch(console.error)
  ...
  const handleReject = async () => {
    try {
      await rejectActivation(rejectModal.id, rejectModal.reason);
  ...
  const actColumns = [
    { key: 'userId', label: 'User', render: (row) => (
      <span className="text-[var(--text-primary)]">{row.userId?.displayName || row.userId?.phone || 'N/A'}</span>
    )},
      ...
      <Card>
        ...
        <div className="border-t pt-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <p className="text-xs text-[var(--text-muted)]">1 USD = {rate.toFixed(2)} {settings.planCurrency}</p>
          </div>
      ...
      <Card>
        <h3 className="font-semibold text-[var(--text-primary)] mb-4">
          ...
          {activations.length > 0 && (
            <Badge variant="warning" className="ml-2">{activations.length}</Badge>
          )}
        </h3>
