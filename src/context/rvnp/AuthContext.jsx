...
import { login as loginApi, setAuthToken, setupInterceptors } from '../../services/rvnp';

const AuthContext = createContext(null);

export function AuthProvider({ children }) {
  const [admin, setAdmin] = useState(() => {
    try { return JSON.parse(localStorage.getItem('rvnp_admin')); } catch { return null; }
  });
  const [token, setToken] = useState(localStorage.getItem('rvnp_token') || null);
  ...
  useEffect(() => {
    ...
    setupInterceptors(() => {
      ...
      window.location.href = '/rvnp/login';
    });
    setLoading(false);
  }, []);
