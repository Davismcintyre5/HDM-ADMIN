...
export function SidebarProvider({ children }) {
  const [darkMode, setDarkMode] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('farmvexa_theme') === 'dark' ||
        (!localStorage.getItem('farmvexa_theme') && window.matchMedia('(prefers-color-scheme: dark)').matches);
    }
    return false;
  });
  ...
  useEffect(() => {
    if (darkMode) {
      ...
      localStorage.setItem('farmvexa_theme', 'dark');
    } else {
      document.documentElement.classList.remove('dark');
      localStorage.setItem('farmvexa_theme', 'light');
    }
  }, [darkMode]);
