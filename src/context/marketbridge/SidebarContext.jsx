...
export function SidebarProvider({ children }) {
  const [darkMode, setDarkMode] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('marketbridge_theme') === 'dark' ||
        (!localStorage.getItem('marketbridge_theme') && window.matchMedia('(prefers-color-scheme: dark)').matches);
    }
    return false;
  });
  ...
  useEffect(() => {
    if (darkMode) { document.documentElement.classList.add('dark'); localStorage.setItem('marketbridge_theme', 'dark'); }
    else { document.documentElement.classList.remove('dark'); localStorage.setItem('marketbridge_theme', 'light'); }
  }, [darkMode]);
