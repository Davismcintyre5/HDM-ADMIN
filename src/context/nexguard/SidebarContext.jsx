...
export function SidebarProvider({ children }) {
  const [darkMode, setDarkMode] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('nexguard_theme') === 'dark' ||
        (!localStorage.getItem('nexguard_theme') && window.matchMedia('(prefers-color-scheme: dark)').matches);
    }
    return false;
  });
  ...
  useEffect(() => {
    if (darkMode) { document.documentElement.classList.add('dark'); localStorage.setItem('nexguard_theme', 'dark'); }
    else { document.documentElement.classList.remove('dark'); localStorage.setItem('nexguard_theme', 'light'); }
  }, [darkMode]);
