...
export function ThemeSidebarProvider({ children }) {
  const [darkMode, setDarkMode] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('bridge_theme') === 'dark' ||
        (!localStorage.getItem('bridge_theme') && window.matchMedia('(prefers-color-scheme: dark)').matches);
    }
    return false;
  });
  ...
  useEffect(() => {
    if (darkMode) { document.documentElement.classList.add('dark'); localStorage.setItem('bridge_theme', 'dark'); }
    else { document.documentElement.classList.remove('dark'); localStorage.setItem('bridge_theme', 'light'); }
  }, [darkMode]);
