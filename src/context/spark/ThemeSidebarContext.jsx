...
export function ThemeSidebarProvider({ children }) {
  ...
  useEffect(() => {
    if (darkMode) { document.documentElement.classList.add('dark'); localStorage.setItem('spark_theme', 'dark'); }
    else { document.documentElement.classList.remove('dark'); localStorage.setItem('spark_theme', 'light'); }
  }, [darkMode]);
