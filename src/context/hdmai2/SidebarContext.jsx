...
export function SidebarProvider({ children }) {
  const [darkMode, setDarkMode] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('hdmai2_theme') === 'dark' ||
        (!localStorage.getItem('hdmai2_theme') && window.matchMedia('(prefers-color-scheme: dark)').matches);
    }
    return false;
  });
  ...
  useEffect(() => {
    if (darkMode) {
      ...
      localStorage.setItem('hdmai2_theme', 'dark');
    } else {
      document.documentElement.classList.remove('dark');
      localStorage.setItem('hdmai2_theme', 'light');
    }
  }, [darkMode]);
