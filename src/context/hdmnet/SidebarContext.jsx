...
export function SidebarProvider({ children }) {
  const [darkMode, setDarkMode] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('hdmnet_theme') === 'dark' ||
        (!localStorage.getItem('hdmnet_theme') && window.matchMedia('(prefers-color-scheme: dark)').matches);
    }
    return false;
  });
  ...
  useEffect(() => {
    if (darkMode) {
      ...
      localStorage.setItem('hdmnet_theme', 'dark');
    } else {
      document.documentElement.classList.remove('dark');
      localStorage.setItem('hdmnet_theme', 'light');
    }
  }, [darkMode]);
